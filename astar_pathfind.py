...
class AStarPathfinder:
    ...
    def pathfind(self, cubes, start, end, mark_dirty_callback=None):
        ...
        Args:
            ...
            mark_dirty_callback: Function to call when marking cubes as dirty for redraw
    ...
    def clear_path(self, cubes, mark_dirty_callback=None):
        ...
        for row in cubes:
            for cube in row:
                if cube.type in [4, 5, 6]:  # closed, open, path
                    cube.make_clear()
                    if mark_dirty_callback:
                        mark_dirty_callback(cube)
        
        self.path_surface.fill((0, 0, 0, 0))  # Clear with transparency
        self.path_points = None
